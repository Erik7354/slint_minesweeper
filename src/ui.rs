...
/// * `ms1` - Rc reference to the Minesweeper game state.
///
/// # Returns
///
/// * A closure that handles the tile click logic, taking the x and y coordinates of the clicked tile as arguments.
fn tile_clicked_callback(weak_handle: Weak<MainWindow>, ms1: Arc<Mutex<Minesweeper>>) -> impl Fn(i32, i32) {
    return move |x: i32, y: i32| {
        let mut ms = ms1.lock().unwrap();
